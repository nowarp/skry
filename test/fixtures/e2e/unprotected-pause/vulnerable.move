...
module test::pause_protocol {
    ...
    /// Not for this rule: Has sink but doesn't check pause (pause-check-missing)
    ...
    /// Not for this rule: Admin function with sink doesn't check pause (admin-bypasses-pause)
    ...
    /// VULNERABLE: Public pause control without role check
    // @expect: unprotected-pause
    public fun pause_protocol(config: &mut Config) {
        config.paused = true;
    }

    /// VULNERABLE: Public unpause without role check
    // @expect: unprotected-pause
    ...
    /// Not for this rule: Entry calls helper that has sink but neither checks pause (pause-check-missing)
